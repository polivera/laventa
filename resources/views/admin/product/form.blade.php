                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <div class="w-full mb-[10px]">
                        ...
                        <form class="flex flex-col gap-4" action="{{ url('admin/productos') }}" method="post"
                            ...
                            <div class="flex flex-col">
                                <label class="text-fg" for="category_id">Categoría: <span
                                        class="text-red-600">*</span></label>
                                <select class="rounded-md border border-gray-300 px-4 pb-2 pt-1 bg-white"
                                    name="category_id" id="category_id">
                                    @foreach ($categories as $category)
                                        <option value="{{ $category->id }}"
                                            @if (old('category_id', $category_id) == $category->id) selected @endif>
                                            {{ $category->name }}
                                        </option>
                                    @endforeach
                                    <select>
